...
class BingAdsManager {
  ...
  private async getCampaignRoasViaBulk(client: ClientConfig, campaignId: string): Promise<number | null> {
    const BULK_SOAP = "https://bulk.api.bingads.microsoft.com/Api/Advertiser/CampaignManagement/v13/BulkService.svc";
    const token = await this.getAccessToken();

    const makeEnvelope = (action: string, body: string) => `<s:Envelope xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  <s:Header xmlns="https://bingads.microsoft.com/CampaignManagement/v13">
    <AuthenticationToken>${token}</AuthenticationToken>
    <CustomerAccountId>${client.account_id}</CustomerAccountId>
    <CustomerId>${client.customer_id}</CustomerId>
  ...
  <s:Body>${body}</s:Body>
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        const cols = lines[i].split(",").map((c: string) => c.trim().replace(/^"|"$/g, ""));
        if (cols[typeIdx] !== "Campaign" || cols[idIdx] !== campaignId) continue;
  ...
  async bulkUpdateBudgets(client: ClientConfig, updates: Array<{ campaign_id: string; daily_budget: number }>): Promise<any> {
    ...
    const campaignMap = new Map<string, number>(
      (listResult?.Campaigns ?? []).map((c: any) => [String(c.Id), parseFloat(c.DailyBudget ?? "0")] as [string, number])
    );

    const verified: Array<{ campaign_id: string; daily_budget: number }> = [];
    const failed: Array<{ campaign_id: string; daily_budget: number; actual: number | null }> = [];
  ...
  async getSpendByHour(client: ClientConfig, date: string, campaignIds?: string[]): Promise<any[]> {
    ...
    const byHour: Record<number, { spend: number; clicks: number; impressions: number; conversions: number; cost_micros: number }> = {};
    for (const r of rows) {
      const h = parseInt(r["TimePeriod"] ?? "0");
      const spend = parseFloat(r["Spend"] ?? "0");
      const clicks = parseInt(r["Clicks"] ?? "0");
      if (!byHour[h]) byHour[h] = { spend: 0, clicks: 0, impressions: 0, conversions: 0, cost_micros: 0 };
  ...
  async getBudgetPacing(client: ClientConfig, monthStart: string, monthEnd: string): Promise<any[]> {
  ...
  async getDisapprovedAds(client: ClientConfig): Promise<any[]> {
  ...
  async getRecommendations(client: ClientConfig): Promise<any[]> {
  ...
  async getExperiments(client: ClientConfig): Promise<any[]> {
  ...
  async createExperiment(client: ClientConfig, options: {
    name: string;
    baseCampaignId: string;
    splitPercent: number; // 1-99, % of traffic going to experiment
    startDate?: string;   // YYYY-MM-DD
    endDate?: string;
  }): Promise<any> {
  ...
  try {
    ...
    switch (name) {
      ...
      case "bing_ads_bulk_update_budgets": {
        ...
        const client = resolveClient(args?.account_id as string);
        const result = await adsManager.bulkUpdateBudgets(
          client,
          args?.updates as Array<{ campaign_id: string; daily_budget: number }>,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case "bing_ads_get_spend_by_hour": {
        const client = resolveClient(args?.account_id as string);
        const result = await adsManager.getSpendByHour(
          client,
          args?.date as string,
          args?.campaign_ids as string[] | undefined,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case "bing_ads_get_budget_pacing": {
        const client = resolveClient(args?.account_id as string);
        const result = await adsManager.getBudgetPacing(
          client,
          args?.month_start as string,
          args?.month_end as string,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case "bing_ads_create_experiment": {
        ...
        const result = await adsManager.createExperiment(client, {
          ...
          baseCampaignId: args?.base_campaign_id as string,
          splitPercent: args?.split_percent as number,
