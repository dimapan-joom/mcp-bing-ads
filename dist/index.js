...
class BingAdsManager {
    ...
    // ============================================
    async getCampaignsWithRoas(client, statusFilter = "active") {
        const BULK_SOAP = "https://bulk.api.bingads.microsoft.com/Api/Advertiser/CampaignManagement/v13/BulkService.svc";
        const token = await this.getAccessToken();
        const makeEnvelope = (body) => `<s:Envelope xmlns:i="http://www.w3.org/2001/XMLSchema-instance" xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <s:Body>${body}</s:Body>
        ...
        if (!requestId)
            throw new Error("Bulk download failed: no request ID");
        ...
        if (!downloadUrl)
            throw new Error("Bulk download timed out after 60s");
    ...
    async listCampaigns(client) {
        ...
        const results = await Promise.all(types.map(type => this.apiCall(url, { AccountId: client.account_id, CampaignType: type }, client, "listCampaigns")
            .catch(() => ({ Campaigns: [] }))));
    ...
    // Handles escaped quote (double quote "") inside quoted fields, e.g. '"Campaign ""A, B"""'
    parseCsvLine(line) {
        const fields = [];
        let current = "";
        let inQuotes = false;
    ...
    // ============================================
    async soapCall(soapAction, bodyXml, client) {
        const token = await this.getAccessToken();
  ...
  <s:Body>${bodyXml}</s:Body>
        ...
        const text = await resp.text();
        if (!resp.ok)
            throw new Error(`SOAP ${soapAction} failed ${resp.status}: ${text.slice(0, 500)}`);
  ...
  <s:Body>${body}</s:Body>
    ...
    // ============================================
    async setCampaignBidding(client, campaignId, strategy) {
        let schemeXml;
    ...
    // SET CAMPAIGN STATUS
    // ============================================
    async setCampaignStatus(client, campaignId, status) {
        const body = `<UpdateCampaignsRequest xmlns="https://bingads.microsoft.com/CampaignManagement/v13">
      <AccountId>${client.account_id}</AccountId>
    ...
    // ============================================
    async addResponsiveSearchAd(client, adGroupId, ad) {
        const headlinesXml = ad.headlines.map((h, i) => `<AssetLink><Asset i:type="TextAsset"><Text>${h.replace(/&/g, "&amp;").replace(/</g, "&lt;")}</Text></Asset><PinnedField i:nil="true"/><AssetPerformanceLabel>Unrated</AssetPerformanceLabel></AssetLink>`).join("");
        const descriptionsXml = ad.descriptions.map(d => `<AssetLink><Asset i:type="TextAsset"><Text>${d.replace(/&/g, "&amp;").replace(/</g, "&lt;")}</Text></Asset><PinnedField i:nil="true"/><AssetPerformanceLabel>Unrated</AssetPerformanceLabel></AssetLink>`).join("");
    ...
    async getMerchantCenterHealth(client) {
        ...
        const storesResp = await this.apiCall(`${CAMPAIGN_MGMT_BASE}/BMCStores/QueryByCustomerId`, {}, client, "getMerchantCenterHealth");
        const stores = storesResp?.BMCStores ?? [];
        const results = [];
        for (const store of stores) {
            const storeId = store.Id;
            const storeName = store.Name;
            if (!store.IsActive || !store.HasCatalog)
                continue;
            // 2. Get catalogs for this store
            let catalogs = [];
            try {
                const catResp = await fetch(`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/${storeId}/catalogs`, { headers: contentHeaders });
            ...
            for (const cat of catalogs) {
                if (!cat.isPublishingEnabled)
                    continue;
                try {
                    const statusResp = await fetch(`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/${storeId}/catalogs/${cat.id}/status`, { headers: contentHeaders });
            ...
            if (totalPublished === 0 && totalRejected === 0)
                continue;
            ...
            const issues = {};
            try {
                const prodResp = await fetch(`https://content.api.bingads.microsoft.com/shopping/v9.1/bmc/${storeId}/products?max-results=200`, { headers: contentHeaders });
                const prodData = await prodResp.json();
                const products = prodData?.resources ?? [];
    ...
    try {
        ...
        switch (name) {
            ...
            case "bing_ads_get_campaigns_with_roas": {
                ...
                const result = await adsManager.getCampaignsWithRoas(client, args?.status_filter ?? "active");
            ...
            case "bing_ads_list_campaigns": {
                ...
                const statusFilter = (args?.status_filter || "active").toLowerCase();
                const filtered = statusFilter === "all"
                    ? result
                    : { ...result, Campaigns: (result?.Campaigns ?? []).filter((c) => c.Status?.toLowerCase() === statusFilter) };
                return {
                    content: [{
                            type: "text",
            ...
            case "bing_ads_get_campaign_performance": {
                ...
                const cpStatusFilter = (args?.status_filter || "active").toLowerCase();
                const filteredResult = cpStatusFilter === "all"
                    ? result
                    : result.filter((r) => r.CampaignStatus?.toLowerCase() === cpStatusFilter);
                return {
                    content: [{
                            type: "text",
            ...
            case "bing_ads_bulk_update_roas": {
                ...
                const client = resolveClient(args?.account_id);
                const result = await adsManager.bulkUpdateRoas(client, args?.updates);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "bing_ads_set_campaign_bidding": {
                assertWriteAllowed("bing_ads_set_campaign_bidding");
                const client = resolveClient(args?.account_id);
                const result = await adsManager.setCampaignBidding(client, args?.campaign_id, {
                    type: args?.strategy_type,
                    targetRoas: args?.target_roas,
                    targetCpa: args?.target_cpa,
                    maxCpc: args?.max_cpc,
                });
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "bing_ads_set_campaign_status": {
                ...
                const client = resolveClient(args?.account_id);
                const result = await adsManager.setCampaignStatus(client, args?.campaign_id, args?.status);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "bing_ads_add_responsive_search_ad": {
                assertWriteAllowed("bing_ads_add_responsive_search_ad");
                const client = resolveClient(args?.account_id);
                const result = await adsManager.addResponsiveSearchAd(client, args?.ad_group_id, {
